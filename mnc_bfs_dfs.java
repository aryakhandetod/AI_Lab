...
public class MissionariesAndCannibals {  
    // State class to represent the state of the 
problem     static class State {         int 
missionariesLeft;         int cannibalsLeft;         int 
missionariesRight;         int cannibalsRight;  
        boolean boatOnLeft;  
  
        public State(int missionariesLeft, int cannibalsLeft, int missionariesRight, int 
cannibalsRight, boolean boatOnLeft) {             this.missionariesLeft = 
missionariesLeft;             this.cannibalsLeft = cannibalsLeft;             
this.missionariesRight = missionariesRight;             this.cannibalsRight = 
cannibalsRight;             this.boatOnLeft = boatOnLeft;  
        }  
  
        // Check if the state is valid         
public boolean isValid() {  
            if (missionariesLeft < 0 || cannibalsLeft < 0 || missionariesRight < 0 || cannibalsRight  
< 0 ||  
...
boatOnLeft == state.boatOnLeft;  
        ...
        @Override  
        public int hashCode() {  
            return Objects.hash(missionariesLeft, cannibalsLeft, missionariesRight, 
cannibalsRight, boatOnLeft);  
        }  
    }  
    ...
    public static void bfs() {  
        ...
        State initialState = new State(3, 3, 0, 0, 
true);         queue.offer(initialState);         
visited.add(initialState);         while 
(!queue.isEmpty()) {  
            State currentState = queue.poll();             
if (currentState.isGoal()) {  
                printSolution(parentMap, 
currentState);                 return;             }  
...
int deltaCL = cannibals[i];             int deltaMR = -deltaML;             int deltaCR = -deltaCL;             
if (currentState.boatOnLeft) {  
                State nextState = new State(currentState.missionariesLeft - deltaML, 
currentState.cannibalsLeft - deltaCL,  
                        currentState.missionariesRight - deltaMR, currentState.cannibalsRight - 
deltaCR, false);  
